    ...
    <div className="min-h-screen flex bg-[#f6f3ef] text-gray-800">
      <aside className={`transition-all duration-200 bg-white border-r border-gray-200 ${collapsed ? 'w-20' : 'w-64'} fixed h-screen overflow-y-auto`}>
        ...
        <nav className="px-2 py-3 space-y-1">
          ...
          {user?.role === 'admin' && <Link to="/admin" className={`flex items-center gap-3 p-2 rounded transition-colors ${collapsed ? '' : 'w-56'} ${location.pathname === '/admin' ? 'bg-amber-50 text-amber-900' : 'hover:bg-gray-50'}`}><span>⚙️</span>{!collapsed && <span>Admin</span>}</Link>}
        </nav>
      </aside>
        ...
        <main className="max-w-7xl mx-auto py-0 sm:px-0 lg:px-0">
          <div className="px-4 py-6 sm:px-0">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
              <Link
                to="/apply-loan"
                className="bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-lg text-center font-medium"
              >
                Apply for Loan
              </Link>
              <Link
